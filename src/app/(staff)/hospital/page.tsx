...
import { AlertOctagon, MapPin, Play, Pause, FileText, CheckCircle, Loader2, Volume2, X, Video, VideoOff, Mic, MicOff, Phone, History, Sparkles, Zap, Bed, CheckCircle2, ArrowRight } from "lucide-react"
import Link from "next/link"
import { getHospitalResourceState, type HospitalResourceState } from "@/lib/resource-optimizer"
...
export default function HospitalPortalPage() {
  ...
  const [resourceState, setResourceState] = useState<HospitalResourceState>(getHospitalResourceState())

  useEffect(() => {
    const handleResUpdate = () => setResourceState(getHospitalResourceState())
    window.addEventListener("storage", handleResUpdate)
    window.addEventListener("medrelay-resource-update", handleResUpdate)
    ...
    return () => { 
      ...
      clearInterval(int2); 
      window.removeEventListener("storage", handleResUpdate);
      window.removeEventListener("medrelay-resource-update", handleResUpdate);
    }
  }, [callStatus])
            ...
            <Card className="shadow-sm border-slate-200 flex-1 flex flex-col min-h-[300px]">
              ...
              <CardContent className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
                ...
                <div className="flex gap-3 justify-end">
                  <div className="bg-slate-900 text-white p-3 rounded-2xl rounded-tr-none text-sm shadow-sm">
                    I see. Can you tell me if your asthma is flaring up?
                  </div>
                  <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-700 shrink-0">Dr</div>
                </div>
              </CardContent>
              <div className="p-4 border-t border-slate-100 bg-white">
                <div className="relative">
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-full py-2.5 pl-4 pr-12 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400"
                  />
                  <button className="absolute right-1 top-1 w-8 h-8 bg-slate-900 rounded-full flex items-center justify-center text-white hover:bg-slate-800 transition-colors">
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      {/* Hospital Resource Allocation Strip (HRAS) */}
      <div className="bg-white rounded-lg p-4 border border-slate-200 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-start gap-3">
          <div className="p-2.5 bg-slate-100 rounded border border-slate-200 text-slate-700 shrink-0">
            <Bed size={20} />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-mono font-bold text-slate-600 uppercase">HRAS Capacity Protocol</span>
              <span className={`text-[10px] font-mono font-bold px-2 py-0.2 rounded border ${resourceState.metrics.isOptimized ? 'bg-emerald-50 text-emerald-800 border-emerald-300' : 'bg-rose-50 text-rose-800 border-rose-300'}`}>
                {resourceState.metrics.isOptimized ? 'CAPACITY BALANCED' : `${resourceState.conflicts.length} BOTTLENECKS ACTIVE`}
              </span>
            </div>
            <h3 className="text-sm font-bold text-slate-900 mt-0.5">Hospital Resource Allocation &amp; Capacity Control</h3>
            <p className="text-xs text-slate-500 mt-0.5 max-w-xl">
              ICU beds, emergency casualty queues, and surgical staff load balancing. Triage wait index: <strong className="text-slate-800">{resourceState.metrics.avgWaitMinutes} mins</strong> • Hospital capacity: {resourceState.metrics.overallEfficiency}%.
            </p>
          </div>
        </div>

        <Link href="/hospital/allocations" className="shrink-0 w-full md:w-auto">
          <Button className="w-full md:w-auto bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs h-8 rounded shadow-xs">
            Capacity &amp; Resource Console →
          </Button>
        </Link>
      </div>
